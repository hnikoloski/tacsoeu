<?php
function cu_wp_custom_breadcrumbs()
{

    $separator              = '<i class="fas fa-chevron-right"></i>';
    $breadcrumbs_id         = 'cu_breadcrumbs';
    $breadcrumbs_class      = 'cu_breadcrumbs';
    $home_title             = esc_html__('Home', 'your-domain');

    // Add here you custom post taxonomies
    $cu_custom_taxonomy    = 'product_cat';
    ...
    if (!is_front_page()) {
        ...
        if (is_archive() && !is_tax() && !is_category() && !is_tag()) {
            ...
            $taxonomy_exists = taxonomy_exists($cu_custom_taxonomy);
            if (empty($last_category) && !empty($cu_custom_taxonomy) && $taxonomy_exists) {

                $taxonomy_terms = get_the_terms($post->ID, $cu_custom_taxonomy);
                $cat_id         = $taxonomy_terms[0]->term_id;
                $cat_nicename   = $taxonomy_terms[0]->slug;
                $cat_link       = get_term_link($taxonomy_terms[0]->term_id, $cu_custom_taxonomy);
